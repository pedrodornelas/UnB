...
static void
UpGoodputSampling (std::string fileName, ApplicationContainer app, Ptr<OutputStreamWrapper> stream, float period)
{
  Simulator::Schedule (Seconds (period), &UpGoodputSampling, fileName, app, stream, period);
  double goodput;
...
static void
DownGoodputSampling (std::string fileName, ApplicationContainer app, Ptr<OutputStreamWrapper> stream, float period)
{
  Simulator::Schedule (Seconds (period), &DownGoodputSampling, fileName, app, stream, period);
  double goodput;
  uint64_t totalPackets = DynamicCast<PacketSink> (app.Get (0))->GetTotalRx ();
  goodput = totalPackets * 8 / (Simulator::Now ().GetSeconds () * 1024); // Kbit/s
  *stream->GetStream () << Simulator::Now ().GetSeconds () << " " << goodput << std::endl;
...
void 
PingRtt (Ptr<OutputStreamWrapper> stream, std::string context, Time rtt)
{
  std::cout << context << "=" << rtt.GetMilliSeconds () << " ms" << std::endl;
  *stream->GetStream () << Simulator::Now ().GetSeconds () << " " << rtt.GetMilliSeconds () << std::endl;
}
...
int main (int argc, char *argv[])
{
  ...
  uint32_t flowsPacketsSize = 1024;

  float startTime = 0.1; // in s
  float simDuration = 100;
  ...
  cmd.AddValue ("nFlows", "Number of Flows", nFlows); // linha adicionada para permitir inserção do número de fluxos no terminal
  ...
  devicesAccessLink.Get (1)->SetAttribute ("ReceiveErrorModel", PointerValue (em1));
  devicesAccessLink.Get (0)->SetAttribute ("ReceiveErrorModel", PointerValue (em1));
  ...
  devicesBottleneckLink.Get (1)->SetAttribute ("ReceiveErrorModel", PointerValue (em1));
  devicesBottleneckLink.Get (0)->SetAttribute ("ReceiveErrorModel", PointerValue (em1));
  ...
  Ptr<OutputStreamWrapper> streamPacketsInQueue = ascii.CreateFileStream (queueDiscType + "-packetsInQueue.txt");
  ...
  for (int i = 0; i < nFlows; i++) //i Flow
  {
      ...
      onOffHelperUp.SetAttribute ("OnTime", StringValue("ns3::NormalRandomVariable[Mean=5.0|Variance=2.0]"));
      onOffHelperUp.SetAttribute ("OffTime", StringValue("ns3::NormalRandomVariable[Mean=8.0|Variance=3.0]"));
  ...
  Ptr<OutputStreamWrapper> v4pingRttStream = ascii.CreateFileStream (queueDiscType + "-v4PingRtt.txt");
  Config::Connect ("/NodeList/*/ApplicationList/*/$ns3::V4Ping/Rtt", MakeBoundCallback (&PingRtt, v4pingRttStream));
  ...
  Simulator::Schedule (Seconds (samplingPeriod), &UpGoodputSampling, queueDiscType + "-upGoodput.txt", uploadApp,
  ...
  Simulator::Schedule (Seconds (samplingPeriod), &DownGoodputSampling, queueDiscType + "-downGoodput.txt", downloadApp,
  ...
  double n_total_packets = 0;
  double down_soma_quadrados=0, down_quadrado_soma=0, justica_down=0;
  double up_soma_quadrados=0, up_quadrado_soma=0, justica_up=0;
  double throughput=0;
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
	{
      ...
      std::cout << "  Taxa Oferecida pela aplicação (Mbps):  " << i->second.txBytes * 8.0 / i->second.timeLastRxPacket.GetSeconds() / 1000 / 1000  << " Mbps\n"; 
      std::cout << "  Número de pacotes perdidos:   " << i->second.lostPackets << "\n";
      std::cout << "  Throughput (Mbps): " << i->second.rxBytes * 8.0 / i->second.timeLastRxPacket.GetSeconds()/ 1000 / 1000  << " Mbps\n";
      std::cout << "  Atraso médio (s): " << i->second.delaySum.GetSeconds()/i->second.rxPackets << "\n";
      std::cout << "  Jitter médio (s): " << i->second.jitterSum.GetSeconds()/i->second.rxPackets << "\n";
      //Calculo da justiça download
      throughput=i->second.rxBytes * 8.0 / i->second.timeLastRxPacket.GetSeconds()/ 1000 / 1000;
      if(t.destinationPort>=100 && t.destinationPort<180){
        down_soma_quadrados+=throughput*throughput;
        down_quadrado_soma+=throughput;
      }
      //Calculo da justiça upload
      else if(t.destinationPort>=20 && t.destinationPort<100){
        up_soma_quadrados+=throughput*throughput;
        up_quadrado_soma+=throughput;
      }
	}

  flowMonitor->SerializeToXmlFile(queueDiscType + "-flowMonitor.xml", true, true);

  justica_down = ((down_quadrado_soma)*(down_quadrado_soma))/((nFlows)*down_soma_quadrados);
	justica_up = ((up_quadrado_soma)*(up_quadrado_soma))/((nFlows)*up_soma_quadrados);

  std::cout << "Justica Download: " << justica_down << "\n";
  std::cout << "Justica Upload: " << justica_up << "\n";
  std::cout << "Total de Pacotes: " << n_total_packets << "\n";
