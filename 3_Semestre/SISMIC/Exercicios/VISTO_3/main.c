...
volatile int conta, soma0, soma1, flag, voltA1, voltA0=0, testa1=0, testa0=0, a, b, cont=0;
...
void anterior(void){
    int a = tens_ant/1000;
    int b = (tens_ant/100)%10;
    int c = (tens_ant%100)/10;
    int d = (tens_ant%100)%10;
    converte(a);
    lcd_char(',');
    converte(b);
    converte(c);
...
void inic_adc(void){
    ...
    ADC12CTL1 = ADC12CSTARTADD_0 | //CSTARADD define primeiro registrador da sequência, portanto, setamos como 0, para ser o primeiro regitrador.
                ...
                ADC12CONSEQ_3;     //Modo repete sequência de canais
    ...
    //Controle da memória 1
    ADC12MCTL1 = ADC12EOS       | //Fim da sequência - Indica último canal a ser convertido
                 ADC12SREF_0    |
                 ADC12INCH_1;     //Canal A1: seleciona P6.1 para fazer conversão

    ADC12IE = ADC12IE0 | ADC12IE1; //habilita interrupção para conversão: ADC12IFG0 e ADC12IFG1
