...
int main (int argc, char *argv[])
{
  ...
  up_soma_quadrados=0, up_quadrado_soma=0, justica_up=0, throughput=0;
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
	{
      ...
      if(t.destinationPort>=8080 && t.destinationPort<8160){
        down_soma_quadrados+=throughput*throughput;
        down_quadrado_soma+=throughput;

      }
      //Calculo da justiça upload
      else if(t.destinationPort>=8000 && t.destinationPort<8080){
